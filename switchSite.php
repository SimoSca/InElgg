...
function switchTest($version, $db, $CONFIG){
  ...
  $based = str_replace('\\', '/', __DIR__);
  $host = 'http://localhost/www/ElggProject';


  // change installation path
  $sql[0] = "UPDATE {$CONFIG->dbprefix}datalists
     SET value = '{$based}/{$version}/'
     WHERE name = 'path'";
  
  // change the data directory
  $sql[1] = "UPDATE {$CONFIG->dbprefix}datalists
     SET value = '{$based}/{$version}-data/'
     WHERE name = 'dataroot'";

  // change the site url
  $sql[2] = "UPDATE {$CONFIG->dbprefix}sites_entity
     SET url = '{$host}/{$version}/'";
...
if(isset($vers)){
  ...
  <!-- <meta HTTP-EQUIV="REFRESH" content="5; url=http://localhost/www/ElggProject/elgg-'.$vers.'/upgrade.php"> -->
  ...
  (function () {
      ...
      var timeDec = function (){
          ...
          if(timeLeft === 0){
              ...
              location.href = "http://localhost/www/ElggProject/<?php echo $vers ?>/upgrade.php";
          }
      };
